...
## 지금까지 완성된 것 (마지막 커밋: 2026-08-07 15:36)
...
- **콘텐츠 재태깅**: 태그로 보기의 사업모델별(성과형/보장형/수익화 하위 상품명)·노출형태별(스플래시·종료팝업·댓글광고 등)·플랫폼별(네이버·토스·배민·인스타그램·트위터 등) 신규 태그는 대부분 0건 상태. 30개 상세페이지 각각에 실제로 어떤 태그가 맞는지 확인해서 연결하는 작업이 남음.
- 재태깅 이후: 메인 LNB(사이드바)를 지금의 "노출형태(피드/전면/배너 등)" 기준에서 사업모델 기준 트리로 바꿀지 여부 논의 예정(펜희님 방침: 태그 재정의 먼저, LNB는 그 다음).
